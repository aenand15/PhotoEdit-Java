...
public class ObservablePictureImpl implements ObservablePicture {
	...
	public ObservablePicture copy(Picture p){
		Picture copied = new PictureImpl(p.getWidth(), p.getHeight());
		for(int i = 0; i < p.getWidth(); i++){
			for(int j = 0; j < p.getHeight(); j++){
				double red = p.getPixel(i, j).getRed();
				double green = p.getPixel(i, j).getGreen();
				double blue = p.getPixel(i, j).getBlue();
				Pixel c = new ColorPixel(red, green, blue);
				copied.setPixel(i, j, c);
			}
		}
		ObservablePicture x = new ObservablePictureImpl(copied);
		return x;
	}
